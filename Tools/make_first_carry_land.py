"""Overlay Carry_Idle right-hand hold onto Land → Carry_Land.

Blender --background --python Tools/make_first_carry_land.py -- FIRST_DIR
"""
import sys
...
carry_path = out_dir / "FirstPlayerCapsule_Carry_Idle.fbx"
source_path = out_dir / "FirstPlayerCapsule_Land.fbx"
clip_name = "Carry_Land"
...
bpy.ops.import_scene.fbx(filepath=str(carry_path), use_anim=True, ignore_leaf_bones=False)
...
bpy.ops.wm.read_factory_settings(use_empty=True)
bpy.ops.import_scene.fbx(filepath=str(source_path), use_anim=True, ignore_leaf_bones=False)
scene = bpy.context.scene
...
# Unity 클립이 0~20을 쓰므로 키를 0부터 잡는다.
...
export_end = last - first
export_fbx(rig, out_dir / f"FirstPlayerCapsule_{clip_name}.fbx", 0, export_end)
print("CARRY_LAND", out_dir, 0, export_end)
