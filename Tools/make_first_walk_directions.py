"""Build Walk_Back / Walk_Left / Walk_Right from Walk_Wide_Clean.

The facing stays forward. Foot swing from the wide walk is remapped onto
back / left / right, matching Crouch_Walk_* directions (Left = +localX).
...
walk_path, out_dir = sys.argv[sys.argv.index("--") + 1 :]
...
bpy.ops.import_scene.fbx(filepath=walk_path, use_anim=True, ignore_leaf_bones=False)
scene = bpy.context.scene
...
clips = (
    ("Walk_Back", Vector((0.0, 0.0, -1.0))),
    ("Walk_Left", Vector((1.0, 0.0, 0.0))),
    ("Walk_Right", Vector((-1.0, 0.0, 0.0))),
)
...
print("WALK_DIRECTION_CLIPS", out_dir)
