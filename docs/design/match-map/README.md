...
- **경계 콜라이더**: `MartEnvironment/Boundary` 아래 6면(West/East/South/North/Ceiling/Floor), 도달 영역 바깥 0.5 m, 두께 2 m — x[-134, -87] z[-27.5, 10] y[-0.5, 7]. 구역이 달라지면 이 값으로 다시 만든다(로비 메뉴처럼 자동화는 아직 안 함).
