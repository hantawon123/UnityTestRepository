...
32자는 컬럼 길이이자 Photon 복제에 쓰는 `NetworkString<_32>` 의 한도입니다. 파츠 id 를
지을 때 이 안에 두어야 저장은 됐는데 남에게는 안 보이는 상태가 생기지 않습니다.
...
남의 외형을 서버에서 조회하는 길은 없습니다. 로비와 경기에서 다른 사람의 외형은 닉네임처럼
Photon 으로 복제합니다. 서버는 내 것만 기억합니다.
