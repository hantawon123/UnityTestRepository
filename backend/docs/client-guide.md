...
> 호출은 이제 필요 없습니다.** 접속자 천 명이면 서버가 초당 33번 DB 에 쓰던 것을 없앤
> 것입니다. 보내야 하는 것은 방을 오갈 때의 한 프레임뿐입니다.
...
조용히 죽는 경우(노트북을 덮거나 Wi-Fi 가 바뀜)는 서버의 ping 이 30초 안에 걸러냅니다.

서버 자체가 죽었다 살아난 경우에만 **90초**가 걸립니다. 그때는 살아 있는 연결이 하나도
없는데 DB 에는 온라인인 행이 남아 있고, 마지막 하트비트가 90초를 넘기면 정리됩니다.
